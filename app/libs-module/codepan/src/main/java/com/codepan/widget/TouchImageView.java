...
import com.codepan.callback.Interface.OnDoubleTapCallback;
import com.codepan.callback.Interface.OnPhotoZoomCallback;
...
public class TouchImageView extends ImageView {
	...
	private OnDoubleTapCallback doubleTapCallback;
	private OnPhotoZoomCallback photoZoomCallback;
	...
	private class ScaleListener extends ScaleGestureDetector.SimpleOnScaleGestureListener {
		...
		public void onScaleEnd(ScaleGestureDetector detector) {
			...
			if(photoZoomCallback != null) {
				if(normalizedScale <= minScale) {
					photoZoomCallback.onPhotoZoom(false);
				}
				else {
					photoZoomCallback.onPhotoZoom(true);
				}
			}
		}
	}
	...
	private void scaleImage(double deltaScale, float focusX, float focusY, boolean stretchImageToSuper) {
		...
		if(photoZoomCallback != null) {
			...
			else if(normalizedScale == maxScale) {
				photoZoomCallback.onPhotoZoom(true);
			}
		}
	...
	private class DoubleTapZoom implements Runnable {
		...
		private static final float ZOOM_TIME = 250;
	...
	public void setOnDoubleTapCallback(OnDoubleTapCallback doubleTapCallback) {
		this.doubleTapCallback = doubleTapCallback;
	}

	public void setOnPhotoZoomCallback(OnPhotoZoomCallback photoZoomCallback) {
		this.photoZoomCallback = photoZoomCallback;
	}
}
